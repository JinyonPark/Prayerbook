-- Reset and bulk-set RPCs, plus execute grants.
...
begin
  ...
  insert into public.prayer_progress_operations (user_id, client_event_id, operation_type)
  values (v_uid, client_event_id, 'current_round_reset')
  returning id into v_op_id;
...
begin
  ...
  insert into public.prayer_progress_operations (user_id, client_event_id, operation_type)
  values (v_uid, client_event_id, 'main_full_reset')
  returning id into v_op_id;
...
begin
  ...
  insert into public.prayer_progress_operations (user_id, client_event_id, operation_type)
  values (v_uid, client_event_id, 'all_full_reset')
  returning id into v_op_id;
...
begin
  ...
  insert into public.prayer_progress_operations (user_id, client_event_id, operation_type)
  values (v_uid, client_event_id, 'bulk_set')
  returning id into v_op_id;
  ...
  insert into public.user_prayer_progress (user_id, prayer_item_id, completion_count, updated_at)
  select v_uid, pi.id, new_count, now()
  from public.prayer_items pi
  where pi.category = 'main'
    and pi.is_active = true
  on conflict (user_id, prayer_item_id)
...
revoke all on function public.internal_progress_snapshot(uuid) from public, anon, authenticated;
revoke all on function public.internal_operation_result(uuid, integer, jsonb) from public, anon, authenticated;
revoke all on function public.internal_existing_operation_result(uuid, uuid) from public, anon, authenticated;
revoke all on function public.internal_require_user() from public, anon, authenticated;
revoke all on function public.internal_current_count(uuid, uuid) from public, anon, authenticated;
revoke all on function public.internal_upsert_count(uuid, uuid, integer) from public, anon, authenticated;

revoke all on function public.get_prayer_progress_summary() from public, anon;
